...
module.exports = tobz = async (tobz, message) => {
    try {
        ...
        const ownerNumber = '6281213082150@c.us'
        ...
        // FUNCTION
                ...
                if(body === '#mute' && isMuted(chatId) == true){
                    if(isGroupMsg) {
                        if (!isAdmin) return tobz.reply(from, 'Maaf, perintah ini hanya dapat dilakukan oleh admin StableBOT!', id)
                ...
                if (body === '#unbanchat') {
                    if (!isOwner) return tobz.reply(from, 'Maaf, perintah ini hanya dapat dilakukan oleh Owner StableBOT!', id)
...
${desc}`)
        ...
        case '#owner':
        case '#creator':
            tobz.sendContact(chatId, `6281213082150@c.us`)
            tobz.reply(from, 'That was my owner number.', id)
            break
        // ON OFF
        case '#nsfw':
            if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
            if (!isOwner) return tobz.reply(from, 'Maaf, perintah ini hanya dapat dilakukan oleh Owner StableBOT! Contact owner di wa.me/6281213082150 untuk mengaktifkan NSFW.', id)
        ...
        case '#simi':
            if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
            if (!isAdmin) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan oleh Admin StableBOT!', id) // Hanya Admin yang bisa mengaktifkan
            if (args.length === 1) return tobz.reply(from, 'Pilih enable atau disable!', id)
            if (args[1].toLowerCase() === 'enable') {
                simi_.push(chat.id)
...
➸ *Wilayah:* +${region_name}`
        ...
        case '#broadcast':
            ...
            for (let ids of chatz) {
                ...
                if (!cvk.isReadOnly) await tobz.sendText(ids, `[ StableBOT BROADCAST ]\n\n${msg}`)
            }
        ...
        case '#ownergroup':
            ...
            break
        case '#otagall': // FOR OWNER & ADMIN StableBOT
        case '#omentionall':
            if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
            if (!isOwner, !isAdmin) return tobz.reply(from, 'Perintah ini hanya untuk Owner StableBOT', id)
        ...
        case '#okickall':
            if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
            if (!isOwner) return tobz.reply(from, 'Perintah ini hanya untuk Admin StableBOT', id)
            if (!isBotGroupAdmins) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan ketika bot menjadi admin', id)
        ...
        case '#oadd':
            ...
            if (!isOwner, !isAdmin) return tobz.reply(from, 'Perintah ini hanya untuk Admin StableBOT', id)
            if (!isBotGroupAdmins) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan ketika bot menjadi admin', id)
            try {
        ...
        case '#oleave':
            if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group', id)
            if (!isOwner, !isAdmin) return tobz.reply(from, 'Perintah ini hanya untuk Admin StableBOT', id)
            await tobz.sendText(from,'StableBOT DIPERINTAHKAN KELUAR OLEH OWNER!!').then(() => tobz.leaveGroup(groupId))
            break
        case '#leave':
            if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group', id)
            if (!isGroupAdmins) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan oleh admin group', id)
        ...
        case '#getses':
            ...
            break
        case '#StableBOTadmin':
            let admn = `This is list of StableBOT Admin\nTotal : ${adminNumber.length}\n`
        ...
        case '#addadmin':
            if (!isOwner) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan oleh Owner StableBOT!', id)
                ...
                tobz.reply(from, 'Success Menambahkan Admin StableBOT!', id)
                }
            break
        case '#deladmin':
            if (!isOwner) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan oleh Owner StableBOT!', id)
        ...
        case '#ping':
            ...
            tobz.sendText(from, `Server RAM Usage: *${(process.memoryUsage().heapUsed / 1024 / 1024).toFixed(2)}MB / ${Math.round(require('os').totalmem / 1024 / 1024)}MB*\nCPU: *${os.cpus().length}*\n\nStatus :\n- *${loadedMsg}* Loaded Messages\n- *${groups.length}* Group Chats\n- *${chatIds.length - groups.length}* Personal Chats\n- *${chatIds.length}* Total Chats\n\nSpeed: ${latensi.toFixed(4)} _Second_`)
            break
        case '#bugreport':
        ...
        case '#help':
            ...
            break
        case '#StableBOTgroup':
            tobz.reply(from, `Link Group StableBOT : https://chat.whatsapp.com/H7bWPfWxeeAKqksE1WEZpX Jangan Lupa Join Ya Gan ${pushname}`, id)
            break
        case '#groupmenu':
        ...
        case '#iklan':
            ...
            break
        case '#adminmenu':
            if (!isAdmin) return tobz.reply(from, 'Perintah ini hanya untuk Admin StableBOT', id)
